...
export default function SuperadminDashboard() {
  ...
  const handlePublish = async () => {
    if (!supabase) {
        showToast("Supabase not connected!"); return;
    }
  ...
  // Fetch data on load so the overview tab has numbers
  useEffect(() => {
    fetchArticles();
  }, []);
        ...
        <header className="h-16 border-b border-slate-800 bg-[#0B0F19] flex items-center justify-between px-4 md:px-8 shrink-0 sticky top-0 z-40">
          ...
          <div className="flex items-center gap-3 md:gap-4">
            <div className="px-2 py-1 md:px-3 md:py-1 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 rounded-full text-[10px] md:text-xs font-bold flex items-center">
              <Activity className="w-3 h-3 mr-1" /> Linked
            </div>
            <div className="w-8 h-8 bg-blue-600 rounded-full flex items-center justify-center text-white font-black text-xs md:text-sm shadow-lg shadow-blue-500/20">CEO</div>
          </div>
        </header>
              ...
              <div className="space-y-6 animate-in fade-in">
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
                  <div className="bg-slate-900 p-5 md:p-6 rounded-2xl border border-slate-800 shadow-lg">
                    ...
                    <div className="text-[10px] md:text-xs text-purple-400 mt-2">Live in Supabase</div>
                  </div>
                  <div className="bg-slate-900 p-5 md:p-6 rounded-2xl border border-slate-800 shadow-lg">
                    ...
                    <div className="text-[10px] md:text-xs text-blue-400 mt-2">Auth coming next phase</div>
                  </div>
                  <div className="bg-slate-900 p-5 md:p-6 rounded-2xl border border-slate-800 shadow-lg col-span-2 md:col-span-1">
                    ...
                    <div className="text-[10px] md:text-xs text-emerald-400 mt-2">Awaiting Stripe integration</div>
                  </div>
                </div>
              </div>
            )}
                  ...
                  <div className="bg-slate-900 rounded-2xl border border-slate-800 overflow-hidden shadow-xl animate-in fade-in">
                    ...
                    {isLoading ? (
                      ...
                      <div className="p-12 text-center text-slate-500 flex flex-col items-center">
                        ...
                        <p className="text-xs mt-2">Switch to 'Write Article' to create one.</p>
                      </div>
                    ) : (
                            ...
                            {articles.map((article) => (
                              <tr key={article.id} className="hover:bg-slate-800/20 transition-colors">
                                ...
                                <td className="px-4 md:px-6 py-3 md:py-4 text-right">
                                  <button onClick={() => handleDelete(article.id)} className="text-red-400 hover:text-red-300 bg-red-400/10 p-2 rounded-lg transition-colors" title="Delete Article">
